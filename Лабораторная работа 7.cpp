...
// Даны две последовательности по 30 целых чисел в каждой.
// Найти наименьшее среди тех чисел первой последовательности, ко¬торые не входят во вторую последовательность, считая, что хотя бы одно число есть.

using namespace std;

// Функция для вывода массива
void print_array(int* matrix, size_t len_mtx) {
...
void gen_array(int* matrix, size_t len_mtx) {
	for (size_t i = 0; i < len_mtx; i++) {
...
void smaller_of_the_two(int* mtx_one, int* mtx_two, size_t len_mtx) {
	int min_number = 0;
	bool min_flag = true;
